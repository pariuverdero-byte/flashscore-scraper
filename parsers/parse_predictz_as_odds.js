...
// PredictZ injection, then multi-source augmentation + Flashscore odds-list fallback.
...
(async () => {
  ...
  for (
    const p of (
      Array.isArray(matched?.selections)
        ? matched.selections
        : []
    )
  ) {
    ...
    const url =
      p.flashscore_url ||
      `https://www.flashscore.mobi/match/${p.flashscore_id}/`;
  ...
  // Single official last-resort fallback:
  // Flashscore Odds list with exact match_id.
  try {
    await import(
