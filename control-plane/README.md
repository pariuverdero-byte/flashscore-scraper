...
- Existing GitHub Actions workflows publish the same generated `signals.json` and `tickets.json` payloads to the control plane; no scraper or second ticket pipeline is involved.
...
Both existing generation workflows upload their native output to `/api/inputs` using the `CONTROL_API_TOKEN` repository secret. The Betfair worker combines these remote snapshots with any local native files and deduplicates them by the existing deterministic intent IDs.

Betfair credentials and certificates are deliberately not used by GitHub-hosted runners because Betfair may reject their foreign data-center location. They remain encrypted on the Romanian worker host. The five-minute GitHub schedule can supply the existing live output, while a locally running live daemon provides the lower-latency path through the same native files.
