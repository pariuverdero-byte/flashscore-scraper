...
export async function POST(request: Request) {
  ...
  const sql = getSql();
  await sql`INSERT INTO bet_transactions (intent_id, kind, event_name, market_text, selection_text, confidence, requested_odds, available_odds, stake, betfair_market_id, betfair_selection_id, betfair_bet_id, raw)
    VALUES (${body.intentId}, ${body.kind}, ${body.eventName}, ${body.marketText}, ${body.selectionText}, ${body.confidence ?? null}, ${body.requestedOdds ?? null}, ${body.availableOdds}, ${body.stake}, ${body.marketId}, ${body.selectionId}, ${body.betId ?? null}, ${JSON.stringify(body.raw ?? {})}::jsonb)
...
export async function PATCH(request: Request) {
  if (!isAuthorized(request)) return Response.json({ error: "Unauthorized" }, { status: 401 });
  await ensureSchema();
  const body = await request.json() as { settlements?: Array<{ betId: string; profit: number; settledAt?: string }> };
  const sql = getSql();
  ...
  return Response.json({ updated: body.settlements?.length ?? 0 });
}
