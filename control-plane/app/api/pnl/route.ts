...
export async function GET(request: Request) {
  ...
  const rows = await getSql()`
    SELECT
      ...
      COALESCE(SUM(profit) FILTER (WHERE status IN ('simulated_won', 'simulated_lost', 'simulated_void') AND submitted_at >= (date_trunc('day', NOW() AT TIME ZONE 'Europe/Bucharest') AT TIME ZONE 'Europe/Bucharest')), 0)::float AS simulated_today,
      COALESCE(SUM(profit) FILTER (WHERE status IN ('simulated_won', 'simulated_lost', 'simulated_void') AND submitted_at >= (date_trunc('month', NOW() AT TIME ZONE 'Europe/Bucharest') AT TIME ZONE 'Europe/Bucharest')), 0)::float AS simulated_month,
      COALESCE(SUM(profit) FILTER (WHERE status IN ('simulated_won', 'simulated_lost', 'simulated_void') AND submitted_at >= (date_trunc('year', NOW() AT TIME ZONE 'Europe/Bucharest') AT TIME ZONE 'Europe/Bucharest')), 0)::float AS simulated_year,
